    ...
    return (
      <section className="px-5 py-16 md:px-10 md:py-20">
        <div className="mx-auto max-w-3xl rounded-3xl border border-sand-200 bg-sand-100 p-8 text-center">
          ...
          <h1 className="mb-4 text-5xl font-light text-forest-900">Thanks for your order.</h1>
          <p className="mb-6 font-sans text-forest-600">
            {submittedOrder.firstName || "Customer"}, your order{" "}
            <strong>{submittedOrder.id}</strong> was placed on {submittedOrder.placedAt}.
          </p>
          <div className="mx-auto mb-7 max-w-sm rounded-2xl border border-sand-300 bg-sand-50 p-4 text-left font-sans text-sm text-forest-700">
            ...
            {submittedOrder.promoCode ? <p>Promo: {submittedOrder.promoCode}</p> : null}
    ...
    return (
      <section className="px-5 py-20 md:px-10">
        <div className="mx-auto max-w-3xl rounded-3xl border border-sand-200 bg-sand-100 p-8 text-center">
          ...
          <p className="mb-7 font-sans text-forest-600">Add products before starting checkout.</p>
          <a
            href="/shop"
            className="inline-flex items-center gap-2 rounded-full bg-forest-900 px-6 py-3 font-sans text-xs font-semibold uppercase tracking-[0.18em] text-sand-50"
          >
  ...
  return (
    <section className="px-5 py-12 md:px-10 md:py-16">
      <div className="mx-auto max-w-7xl">
        ...
        <h1 className="mb-8 text-6xl font-light text-forest-900">Mock Checkout</h1>
        ...
        <div className="grid gap-7 lg:grid-cols-[1.1fr_0.9fr]">
          <form
            ...
            noValidate
          >
            <h2 className="text-4xl font-light text-forest-900">Contact & Shipping</h2>
            ...
            <label className="block">
              ...
              {touched.email && errors.email ? (
                <p className="mt-1.5 font-sans text-xs text-clay-700">{errors.email}</p>
              ) : null}
            </label>
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              <label className="block">
                ...
                {touched.firstName && errors.firstName ? (
                  <p className="mt-1.5 font-sans text-xs text-clay-700">{errors.firstName}</p>
                ) : null}
              </label>
              <label className="block">
                <span className="mb-1.5 block font-sans text-xs font-semibold uppercase tracking-[0.18em] text-forest-600">
                ...
                {touched.lastName && errors.lastName ? (
                  <p className="mt-1.5 font-sans text-xs text-clay-700">{errors.lastName}</p>
                ) : null}
              </label>
            </div>
            ...
            <label className="block">
              ...
              {touched.address && errors.address ? (
                <p className="mt-1.5 font-sans text-xs text-clay-700">{errors.address}</p>
              ) : null}
            </label>
            ...
            <div className="grid gap-4 sm:grid-cols-3">
              <label className="block sm:col-span-2">
                ...
                {touched.city && errors.city ? (
                  <p className="mt-1.5 font-sans text-xs text-clay-700">{errors.city}</p>
                ) : null}
              </label>
              <label className="block">
                <span className="mb-1.5 block font-sans text-xs font-semibold uppercase tracking-[0.18em] text-forest-600">
                ...
                {touched.state && errors.state ? (
                  <p className="mt-1.5 font-sans text-xs text-clay-700">{errors.state}</p>
                ) : null}
              </label>
            </div>
            ...
            <label className="block">
              ...
              {touched.zip && errors.zip ? (
                <p className="mt-1.5 font-sans text-xs text-clay-700">{errors.zip}</p>
              ) : null}
            </label>
            ...
            <fieldset>
              ...
              <div className="grid gap-2">
                <label className="flex items-center justify-between rounded-xl border border-sand-300 bg-sand-50 px-3 py-2.5 font-sans text-sm text-forest-700">
                  ...
                  <span>
                    {$subtotal >= 75 || appliedPromo?.type === "shipping" ? "Free" : formatUSD(7)}
                  </span>
                </label>
                <label className="flex items-center justify-between rounded-xl border border-sand-300 bg-sand-50 px-3 py-2.5 font-sans text-sm text-forest-700">
                  <span>
          ...
          <aside className="h-fit rounded-3xl border border-sand-200 bg-sand-100 p-5">
              ...
              {$lineItems.map((item) => (
                ...
                >
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="font-sans text-xs text-forest-500">Qty {item.quantity}</p>
                  </div>
            ...
            <div className="mb-4 rounded-2xl border border-sand-300 bg-sand-50 p-3">
              ...
              {promoError ? (
                <p className="mt-2 font-sans text-xs text-clay-700">{promoError}</p>
              ) : null}
              {appliedPromo ? (
